...
const orderReducer = (state, action) =>{
    ...
    if(action.type === 'ADD'){
        const existingOrderItemIndex = state.items.findIndex(item => item.id === action.item.id)
        ...
        const updatedTotalQuantity = state.totalQuantity + updatedPrice * action.item.quantity
...
const OrderProvider = (props) => {
    const [orderState, dispatchOrderaction] = useReducer(orderReducer, defaulOrderState)
